...
# arm64 build compatibility inside QEMU on an amd64 host
...
COPY --from=builder --chown=nobody:root /app/_build/${MIX_ENV}/rel/feedproxy ./
COPY entrypoint.sh /
RUN chown -R nobody /entrypoint.sh && chmod +x /entrypoint.sh
